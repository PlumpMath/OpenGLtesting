...
FontDraw::FontDraw(char *todraw, float ax, float ay)
{
	...
	for (int i = 0; i < strlen(textToDraw); i++)
	{
		...
		FontSprites[i].Initialize(tx, y, "resources/defaultfont.png", charload[(int)textToDraw[i]].width, charload[(int)textToDraw[i]].height, 3, getOGL(charload[(int)textToDraw[i]].x, charload[(int)textToDraw[i]].y, charload[(int)textToDraw[i]].width, charload[(int)textToDraw[i]].height));
	}
